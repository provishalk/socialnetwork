...
const Post = ({ name, text, createdAt, likes, postId, postedBy, comments }) => {
    ...
    const onHandleLike = event => {
        ...
        axios
            .get(`${process.env.REACT_APP_BASE_URL}${event.target.src === LIKED ? LIKE_POST : DISLIKE_POST}${postId}`, config)
            .then()
    ...
    const onDeletePostHandler = () => {
        ...
        axios
            .delete(`${process.env.REACT_APP_BASE_URL}${DELETE_POST}${postId}`, config)
            .then()
            .catch(err => console.error(err));
    }
                ...
                <div className="post__user">
                    ...
                    {postedBy._id === user._id ? (
                        <Dropdown className="post__dropdown">
                            <Dropdown.Toggle className="post__dropdown post__dropdowm-btn" variant="light" id="dropdown-basic" size="sm">
                            </Dropdown.Toggle>
                            <Dropdown.Menu>
                                <Dropdown.Item onClick={onDeletePostHandler}>Delete</Dropdown.Item>
                            </Dropdown.Menu>
                        </Dropdown>
                    ) : (<></>)}
                </div>
